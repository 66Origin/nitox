// Taken from redis-async-rs without any shame
/*
    ...
    CE: std_error::Error + 'static,
{
    ...
    fn reconnect(&self) -> impl Future<Item = (), Error = ()> {
        ...
        *state = ReconnectState::Connecting;
        ...
        executor.spawn(Box::new(connect_f)).expect("Cannot spawn future");
...
impl<A, T, RE, CE> Clone for Reconnect<A, T, RE, CE> {
    fn clone(&self) -> Self {
        // Always shuffle cluster addresses otherwise we always connect to the same node
        let mut cluster = self.cluster.clone();
        thread_rng().shuffle(&mut cluster);
